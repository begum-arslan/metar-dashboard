...
export default function HeadTailWindPctTab({ data }) {
  ...
  const [minSpeedInput, setMinSpeedInput] = useState('15');
  const [maxSpeedInput, setMaxSpeedInput] = useState('25');
    ...
    parsed.forEach(d => {
      ...
      if (key !== null) {
        ...
        if (appliedFilters !== null) {
          ...
          if (typeof d.windDirection === 'number') {
            ...
            // Criteria bounds
            if (valueToCheck > 0 && valueToCheck >= minSpeed && valueToCheck <= maxSpeed) {
        ...
        <div className="glass-container md:col-span-3" style={{ padding: '24px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <div style={{ fontWeight: 600, color: 'var(--text-muted)', fontSize: '0.85rem' }}>
              Head Tail Wind % (Totals For: Records / Criteria / Metar: {totalRecords} / {totalCriteria} / {totalMetar})
            </div>
          </div>
          
          {appliedFilters === null ? (
            <div style={{ height: '350px', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--text-muted)' }}>
              Lütfen sol panelden rüzgar kriterlerini belirleyip Run butonuna basın.
            </div>
          ) : (

...
export default function TemperaturePctTab({ data, reportInfo }) {
  ...
  const handleRun = () => {
    const minVal = parseFloat(minTempInput);
    const maxVal = parseFloat(maxTempInput);
    
    if (!isNaN(minVal) && !isNaN(maxVal)) {
      setAppliedFilters({ minTemp: minVal, maxTemp: maxVal });
    } else {
      ...
      if (key !== null) {
        ...
        if (appliedFilters !== null) {
          ...
          if (d.temperature !== null && d.temperature !== undefined) {
            if (d.temperature >= minTemp && d.temperature <= maxTemp) {
        ...
        <div className="glass-container" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
            <button 
              ...
              onClick={() => exportGraphAsPNG(chartRef, 'TemperaturePctTab.png')}
            >
              📈 Export Graph
            </button>
            <button 
        ...
        <div ref={chartRef} className="glass-container md:col-span-3" style={{ padding: '24px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <div style={{ fontWeight: 600, color: 'var(--text-muted)', fontSize: '0.85rem' }}>
              Temperature % (Totals For: Records / Criteria / Metar: {totalRecords} / {totalCriteria} / {totalMetar})
            </div>
          </div>
